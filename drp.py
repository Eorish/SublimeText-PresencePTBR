...
def base_activity(started = False):
    activity = {
        ...
        'state': settings.get('start_state') if started else 'Ausente'
    }
...
def get_icon(file, ext, _scope):
    ...
    logger.debug('Usando o ícone "%s" Abriu a Pasta %s (scope: %s)', icon, file, main_scope)
...
def handle_activity(view, is_write=False, idle=False):
    ...
    if last_file != entity and settings.get('time_per_file'):
        logger.info('adicionando novo carimbo de data/hora.')
        stamp = mktime(time.localtime())

    logger.info('Upload de Atividade')
...
def handle_error(exc, retry=True):
    sublime.active_window().status_message("[DiscordRP] Falha no envio da atividade.")
    logger.error("Falha no envio da atividade. Error: %s", exc)
    disconnect()
...
def get_project_name(window, current_file):
    ...
    for source in sources:
        if source == "project_folder_name":
            ...
            logger.error("Editando Font `project_name` contexto: %r", source)
...
def connect(silent=False, retry=True):
    global ipc
    if ipc:
        logger.error("Já está conectado")
        return True

    try:
        ipc = discord_ipc.DiscordIpcClient.for_platform(DISCORD_CLIENT_ID)
    except (OSError, discord_ipc.DiscordIpcError) as e:
        logger.info("Não foi possível conectar ao cliente Discord")
        logger.debug("Erro ao conectar", exc_info=e)
        if not silent:
            sublime.error_message("[DiscordRP] Não foi possível conectar ao cliente Discord."
                                  "\n\nVerifique se ele está em execução."
                                  " Corre 'Discord Rich Presence: Conecte-se ao Discord'"
                                  " Tentar novamente.")
        if retry:
            global is_connecting
            is_connecting = True
...
def connect_background():
    if not is_connecting:
        logger.warning("Nova tentativa de conexão automática cancelada!")
        return

    logger.info("Tentando se reconectar ao cliente Discord...")
